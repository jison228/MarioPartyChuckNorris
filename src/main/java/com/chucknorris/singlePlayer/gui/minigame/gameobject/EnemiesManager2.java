...
public class EnemiesManager2 {
	...
	public EnemiesManager2(MainCharacter2 mainCharacter, int tier, int dificultadIA) {
		rand = new Random();
	...
	public void update() {
		...
		if (tier < dificultadIA) {
			if (tier < 1) {
				if (enemy.getBound().intersectsLine(160.0, 0.0, 160.0, 600.0)) {
					mainCharacter.jump();
				}
			} else if (tier < 4) {
				if (enemy.getBound().intersectsLine(220.0, 0.0, 220.0, 600.0)) {
					mainCharacter.jump();
				}
			} else if (tier < 7) {
				if (enemy.getBound().intersectsLine(300.0, 0.0, 300.0, 600.0)) {
					mainCharacter.jump();
				}

			} else {
				if (enemy.getBound().intersectsLine(325.0, 0.0, 325.0, 600.0)) {
					mainCharacter.jump();
				}
			}
